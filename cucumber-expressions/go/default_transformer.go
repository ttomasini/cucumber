...
type SimpleDefaultTransformer struct {
}

func (s SimpleDefaultTransformer) Transform(fromValue string, toValueType interface{}) (interface{}, error) {
...
func createError(fromValue string, toValueType interface{}) error {
	return errors.New(fmt.Sprintf("Can't transform '%s' to %s. "+
		"SimpleDefaultTransformer only supports a limited number of types. "+
